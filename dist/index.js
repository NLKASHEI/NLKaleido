...
const derivedExprSchema = unionType([
    predicateExprSchema,
    objectType({ fn: enumType(['sum', 'count', 'concat', 'custom']), args: arrayType(stringType().min(1)).optional() }),
]);
const derivedFieldSchema = objectType({
    path: stringType().min(1),
    expr: derivedExprSchema,
    persist: enumType(['run', 'global']).optional(),
});
const sideEffectActionSchema = objectType({
    action: enumType(['setStVar', 'emitEvent', 'queueWorldInfo', 'callHook', 'sendNotification']),
    payload: recordType(stringType(), unknownType()),
});
const sideEffectSchema = objectType({
    id: stringType().min(1),
    when: predicateExprSchema,
    then: arrayType(sideEffectActionSchema).min(1),
    throttle: numberType().int().nonnegative().optional(),
});
const middlewarePointSchema = enumType([
    'message.received', 'request.build', 'request.settingsReady',
    'ops.beforeValidate', 'ops.afterValidate', 'ops.beforeCommit', 'ops.afterCommit',
    'state.read', 'state.changed', 'render.stateTable', 'render.tail',
    'cache.prefix', 'cache.miss', 'persist.before', 'persist.after',
    'agent.visible', 'agent.decide',
]);
const middlewareRefSchema = objectType({ point: middlewarePointSchema, name: stringType().min(1) });
...
const contractSchema = objectType({
    ...
    derived: recordType(stringType(), derivedFieldSchema).optional(),
    sideEffects: arrayType(sideEffectSchema).optional(),
    middleware: arrayType(middlewareRefSchema).optional(),
...
function validateContract(c, opts) {
    ...
    // 7. 派生/副作用声明（不再接受 unknown 占位）。
    for (const [name, derived] of Object.entries(c.derived ?? {})) {
        const pathError = validateFieldPath(derived.path);
        if (pathError)
            errors.push(`derived '${name}' 路径非法（${derived.path}）：${pathError}`);
        if ('fn' in derived.expr && derived.expr.fn === 'custom') {
            const fnName = derived.expr.args?.[0];
            if (!fnName)
                errors.push(`derived '${name}'：custom 必须把注册函数名放在 args[0]`);
            else if (opts?.registry) {
                try {
                    opts.registry.resolve('derivedFn', fnName);
                }
                catch {
                    errors.push(`derived '${name}' 引用了未注册函数：${fnName}`);
                }
            }
        }
    }
    ...
    for (const effect of c.sideEffects ?? []) {
        if (effectIds.has(effect.id))
            errors.push(`sideEffect id 重复：${effect.id}`);
...
/** §18.8 Scheduler 默认实现（覆盖即默认，§0.1-C） */
const DueFieldScheduler = Object.freeze({
    id: 'builtin:dueFields',
    due: (meta, c, turnId) => dueFields(meta, c, turnId),
});
/**
...
/** §18.5 默认渲染后端（replaces:'builtin:render'，覆盖即默认） */
const TextBackend = Object.freeze({
    id: 'builtin:render',
    render: (view, _ctx) => {
    ...
    estimateTokens: (view) => estimateTokens(view.fields.map((f) => `${f.path}: ${JSON.stringify(f.value)}`).join('\n')),
});
 ...
 */
/** §18.7 默认缓存策略；作者可通过 registry replaces:'builtin:cacheStrategy' 整体覆盖。 */
const DefaultCacheStrategy = Object.freeze({
    id: 'builtin:cacheStrategy',
    buildPrefix(input) {
        return buildPrefixSegments(input.contract, input.state, input.due, { l0: input.l0, contractId: input.contract.id });
    },
    onMiss() { },
    onHit() { },
});
...
// CacheKey 组装（§7）
// ============================================================
function buildCacheKey(cardFingerprint, presetFingerprint, contractVersion, segments) {
    return {
        cardFingerprint,
        presetFingerprint,
        contractVersion,
        segmentHashes: segments.map((s) => s.fingerprint),
    };
}
...
function validateOps(c, state, ops, turnId, opts = {}) {
    ...
    for (const op of ops) {
        try {
            ...
            if (field.updateMode === 'fixed' && opts.allowFixed !== true) {
...
function applyOps(state, applied, turnId, opts = {}) {
    ...
    for (const op of applied) {
        ...
        const dup = opts.dedupe !== false && source !== 'rollback'
...
async function runSingleShotAgent(transport, opts) {
    ...
    await opts.runtime?.run('state.read', contract, state, turnId, { due });
    ...
    await opts.runtime?.run('agent.visible', contract, state, turnId, observation);
    ...
    const prefix = opts.runtime?.cacheStrategy.buildPrefix({ contract, state, due, l0: opts.l0 })
        ?? buildPrefixSegments(contract, state, due, { l0: opts.l0 });
    const { segments } = prefix;
    await opts.runtime?.run('cache.prefix', contract, state, turnId, prefix);
    // 4a. L3「记忆段」（§20.6：检索 top-K 注入；预算与观察层共用 maxStatusTokens，前缀不受影响）
    const memorySegment = opts.memorySegment ?? null;
    ...
    await opts.runtime?.run('render.tail', contract, state, turnId, tail);
    const messages = [...segments.map((s) => ({ role: 'system', content: s.content })), ...tail];
    const cacheKey = buildCacheKey(contract.id, 'active-preset', contract.version, segments);
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt += 1) {
        ...
        const requestMessages = feedbackText
            ...
            : messages;
        await opts.runtime?.run('request.build', contract, state, turnId, { messages: requestMessages, attempt });
        await opts.runtime?.run('agent.decide', contract, state, turnId, { messages: requestMessages, attempt });
        ...
        if (opts.runtime) {
            ...
            const hitTokens = Number(usage.prompt_cache_hit_tokens ?? usage.cached_tokens ?? usage.cache_read_input_tokens ?? 0);
            if (hitTokens > 0) {
                opts.runtime.cacheStrategy.onHit({ key: cacheKey, hitTokens });
            }
            else {
                const miss = { key: cacheKey, segments };
                opts.runtime.cacheStrategy.onMiss(miss);
                await opts.runtime.run('cache.miss', contract, state, turnId, miss);
            }
        }
        // 5. 解析 + 语法清洗（§4.8 ①）
        ...
        await opts.runtime?.run('ops.beforeValidate', contract, state, turnId, { ops });
        const validation = opts.runtime?.validator(contract, state, ops, turnId, { source: 'agent' })
            ?? validateOps(contract, state, ops, turnId, { source: 'agent' });
        const { applied, rejected } = validation;
        await opts.runtime?.run('ops.afterValidate', contract, state, turnId, validation);
        ...
        // 7. 应用 + changelog
        await opts.runtime?.run('ops.beforeCommit', contract, state, turnId, { ops: gated });
        applyOps(state, gated, turnId, { source: 'agent' });
        await opts.runtime?.run('ops.afterCommit', contract, state, turnId, { ops: gated });
        await opts.runtime?.run('state.changed', contract, state, turnId, { paths: gated.map((op) => op.path) });
...
async function runMultiStepAgent(transport, opts) {
    ...
    for (let step = 0; step < maxSteps; step += 1) {
        ...
        if (stepTokens > maxTokensPerStep || result.tokensConsumed + stepTokens > totalBudget) {
            ...
            result.terminationReason = `token 预算耗尽：本步 ${stepTokens}/${maxTokensPerStep}，累计 ${result.tokensConsumed}/${totalBudget}`;
            break;
        }
        result.tokensConsumed += stepTokens;
        if (decision.action === 'done') {
            result.status = 'done';
            result.terminationReason = decision.rationale || 'agent 判定完成';
            break;
        }
        if (decision.action === 'get_state') {
            stateView = await getState();
            feedback = 'get_state 已刷新。下一步请提交 apply_patch 或 done。';
            history.push({ action: 'get_state', result: stateView });
            continue;
        }
        const { ops } = sanitizeJsonPatchDetailed(decision.ops, contract);
        if (!ops.length) {
            feedback = 'apply_patch 未包含任何合法操作；请修正，或在无需修改时返回 done。';
            history.push({ action: 'apply_patch', result: feedback });
            continue;
        }
        ...
        if (toolResult.ok) {
            ...
            stateView = await getState();
            feedback = 'apply_patch 成功。请检查最新状态，继续修改或返回 done。';
            history.push({ action: 'apply_patch', result: feedback });
            continue;
        }
        ...
        feedback = `apply_patch 被拒绝：${toolResult.errors?.join('；') ?? '未知错误'}。请依据约束修正，不要重复同一操作。`;
        history.push({ action: 'apply_patch', result: feedback });
    }
...
/** 声明式派生与副作用判定；Core 只产出确定性结果/动作意图，不执行任何 ST IO。 */
function evaluateDerivedFields(contract, state, resolveCustom) {
    const changes = [];
    for (const [name, field] of Object.entries(contract.derived ?? {})) {
        ...
        if ('fn' in expr) {
            ...
            if (expr.fn === 'custom') {
                const fnName = args[0];
                if (!fnName || !resolveCustom)
                    throw new Error(`derived '${name}' 缺少可用的自定义函数：${String(fnName)}`);
                const values = args.slice(1).map((path) => getAtPath(state.stat_data, path));
                newValue = resolveCustom(fnName)(values, { field, state, contract });
            }
            else {
                const values = args.map((path) => getAtPath(state.stat_data, path));
                if (expr.fn === 'sum')
                    newValue = values.reduce((sum, value) => sum + (typeof value === 'number' && Number.isFinite(value) ? value : 0), 0);
                else if (expr.fn === 'count')
                    newValue = values.reduce((count, value) => {
                    ...
                    newValue = values.map((value) => value == null ? '' : String(value)).join('');
            }
        }
        else {
            newValue = evaluatePredicate(expr, { state, sources: {} });
        }
        const oldValue = getAtPath(state.stat_data, field.path);
        if (JSON.stringify(oldValue) !== JSON.stringify(newValue)) {
            setAtPath(state.stat_data, field.path, newValue);
            changes.push({ name, path: field.path, oldValue, newValue });
        }
    }
    return changes;
}
function collectSideEffectIntents(contract, state, sources, lastTriggeredAt, now) {
    const intents = [];
    const triggeredAt = { ...lastTriggeredAt };
    const errors = [];
    for (const effect of contract.sideEffects ?? []) {
        try {
            if ((effect.throttle ?? 0) > 0 && now - (lastTriggeredAt[effect.id] ?? Number.NEGATIVE_INFINITY) < (effect.throttle ?? 0))
                continue;
            if (!evaluatePredicate(effect.when, { state, sources }))
                continue;
            triggeredAt[effect.id] = now;
            for (const action of effect.then)
                intents.push({ effectId: effect.id, action: action.action, payload: action.payload });
        }
        catch (error) {
            errors.push(`${effect.id}：${error instanceof Error ? error.message : String(error)}`);
        }
    }
    return { intents, triggeredAt, errors };
}

/** 存储键独立模块，避免 Adapter ↔ StorageProvider 循环依赖。 */
...
/** chat 层默认存储后端（replaces:'builtin:storage.chat'，§18.6） */
function createChatMetadataProvider(globals) {
    return {
        id: 'builtin:storage.chat',
        async load(key) {
            const meta = globals.getChatMetadata();
            const variables = (meta.variables ?? {});
            return variables[key] ?? null;
        },
        async save(key, value, _meta) {
            const meta = globals.getChatMetadata();
            const variables = { ...(meta.variables ?? {}), [key]: value };
            meta.variables = variables;
            globals.setChatMetadata(meta);
            globals.saveChat(); // U2 补充：须自行落盘
        },
        async list(prefix) {
            const meta = globals.getChatMetadata();
            const variables = (meta.variables ?? {});
            return Object.keys(variables).filter((k) => k.startsWith(prefix));
        },
    };
}
/** run/global 层默认存储后端（replaces:'builtin:storage.global'，§16.1/§18.6） */
function createGlobalSettingsProvider(globals) {
    return {
        id: 'builtin:storage.global',
        async load(key) {
            const ext = globals.getExtensionSettings();
        ...
        async save(key, value, _meta) {
            const ext = globals.getExtensionSettings();
            const variables = { ...(ext.variables ?? {}) };
            const globalStore = { ...(variables.global ?? {}), [key]: value };
            variables.global = globalStore;
            ext.variables = variables;
            globals.saveSettings(); // U6：debounce → POST /api/settings/save
        },
        async list(prefix) {
            const ext = globals.getExtensionSettings();
            const variables = (ext.variables ?? {});
            const globalStore = (variables.global ?? {});
            return Object.keys(globalStore).filter((k) => k.startsWith(prefix));
        },
    };
}
/** 按 persist 作用域路由（§16.1：chat → ChatMetadata；run/global → GlobalSettings） */
function createStorageRouter(globals) {
    const chat = createChatMetadataProvider(globals);
    const global = createGlobalSettingsProvider(globals);
    const chatKeys = new Set([STORE_KEY_CHAT, STORE_KEY_CHAT_MEMORY, STORE_KEY_CHAT_PLOT, STORE_KEY_CHAT_DICE]);
    const providerForKey = (key) => chatKeys.has(key) ? chat : global;
    const unified = {
        id: 'builtin:storageProvider',
        load: (key) => providerForKey(key).load(key),
        save: (key, value, meta) => providerForKey(key).save(key, value, meta),
        async list(prefix) {
            const [chatKeysFound, globalKeysFound] = await Promise.all([chat.list(prefix), global.list(prefix)]);
            return [...new Set([...chatKeysFound, ...globalKeysFound])];
        },
    };
    return {
        chat,
        global,
        unified,
        forScope: (scope) => (scope === 'chat' ? chat : global),
        chatKey: STORE_KEY_CHAT,
        globalKey: (contractId) => `${STORE_KEY_GLOBAL_PREFIX}${contractId}`,
        globalSharedKey: `${STORE_KEY_GLOBAL_PREFIX}global`,
    };
}
...
class KaleidoRegistryImpl {
    entries = new Map();
    counter = 0;
    register(kind, name, impl, meta) {
        const list = this.entries.get(kind) ?? [];
        ...
        this.entries.set(kind, list);
    }
    /**
     * 解析组件。
     * - 给 name：精确匹配（作者/内建同名后注册者覆盖，§18.3 覆盖即默认）。
     * - 不给 name：取该 kind 的默认项——「replaces 命中 builtin 的作者项」优先于内建默认，
     *   同 replaces 按 priority/order 取后者（§18.4 覆盖即默认）。
     */
    resolve(kind, name) {
        const list = this.entries.get(kind) ?? [];
        if (name !== undefined) {
            ...
            if (!found)
                throw new RegistryError(`组件未注册：${kind}/${name}`);
            return found.impl;
        }
        if (!list.length)
            throw new RegistryError(`组件种类未注册：${kind}`);
        const builtinName = `builtin:${kind}`;
        // 覆盖项优先：replaces 指向 builtin 的项（按 priority 降序、order 降序 = 后注册者胜）
        const overrides = list.filter((e) => e.replaces === builtinName);
        const chosen = overrides.length
            ? overrides.sort((a, b) => b.priority - a.priority || b.order - a.order)[0]
            : list.sort((a, b) => b.priority - a.priority || b.order - a.order)[0];
        return chosen.impl;
    }
    list(kind) {
        return (this.entries.get(kind) ?? [])
            ...
            .map((e) => ({ name: e.name, replaces: e.replaces, priority: e.priority }));
    }
    remove(kind, name) {
        const list = this.entries.get(kind) ?? [];
    ...
    /** 可序列化快照（注册表持久化，§8 一致性「注册自定义中间件/组件」行） */
    snapshot() {
        const out = [];
        for (const [kind, list] of this.entries) {
            for (const e of list) {
                out.push({ kind, name: e.name, replaces: e.replaces, priority: e.priority });
            }
        }
        return out;
    }
    /** 注册内建默认项（幂等；replaces:'builtin:xxx' 约定见 §18.4） */
    registerBuiltin(kind, impl, opts) {
        this.register(kind, `builtin:${kind}`, impl, { priority: opts?.priority ?? 0 });
    }
}
class MiddlewarePipeline {
    byPoint = new Map();
    counter = 0;
    /** 注册中间件（§18.3：优先级排序；同 priority 后注册者靠内层） */
    use(point, name, fn, priority = 0) {
        const list = this.byPoint.get(point) ?? [];
        list.push({ name, fn, priority, order: this.counter += 1 });
    ...
    list(point) {
        return (this.byPoint.get(point) ?? []).map((m) => ({ name: m.name, priority: m.priority }));
    }
    /**
     * 洋葱执行：外层（高优先级）先进入、后退出；内层可经 next() 委托默认实现（§18.4 链式回退）。
     * 错误隔离（§18.3）：单个中间件抛错被捕获记录，不炸整链（对齐 §17.4 错误隔离）。
     */
    async run(point, ctx) {
        const list = this.byPoint.get(point) ?? [];
        const errors = [];
        const dispatch = (index) => {
            ...
            return Promise.resolve()
                .then(() => registration.fn(ctx, () => dispatch(index + 1)))
                .catch((error) => {
                errors.push(`${registration.name}：${error instanceof Error ? error.message : String(error)}`);
                return dispatch(index + 1); // 跳过出错中间件，继续链（不炸整链）
            });
        };
        await dispatch(0);
...
/** 契约 middleware 引用解析（§7 MiddlewareRef → 注册表中间件；未注册 → 校验失败明确报错，§18.12） */
function resolveContractMiddleware(refs, pipeline) {
    const errors = [];
    for (const ref of refs ?? []) {
        const registered = pipeline.list(ref.point).some((m) => m.name === ref.name);
        if (!registered) {
            errors.push(`契约引用的中间件未注册：${ref.point} / ${ref.name}`);
        }
    }
    return errors;
}
...
/**
 * 生产用扩展运行时：注册表与 17 个中间件点不再只是单测孤岛。
 * 内建实现也通过注册表解析，因此 replaces:'builtin:<kind>' 会真实改变主链路。
 */
class ExtensionRuntime {
    registry = new KaleidoRegistryImpl();
    middleware = new MiddlewarePipeline();
    constructor(storageProviders = [], defaultStorageProvider) {
        this.registry.registerBuiltin('scheduler', DueFieldScheduler);
        this.registry.registerBuiltin('validator', validateOps);
        this.registry.registerBuiltin('renderBackend', TextBackend);
        this.registry.registerBuiltin('cacheStrategy', DefaultCacheStrategy);
        if (defaultStorageProvider) {
            this.registry.registerBuiltin('storageProvider', defaultStorageProvider);
        }
        for (const provider of storageProviders) {
            this.registry.register('storageProvider', provider.id, provider, {
                // 具名 provider 供精确 resolve 使用；默认解析必须落到组合 builtin，避免只选中 chat/global 半边。
                priority: -100,
            });
        }
    }
    register(kind, name, impl, meta) {
        this.registry.register(kind, name, impl, meta);
    }
    use(point, name, fn, priority = 0) {
        this.middleware.use(point, name, fn, priority);
    }
    remove(kindOrPoint, name) {
        if (kindOrPoint.includes('.'))
            return this.middleware.remove(kindOrPoint, name);
        return this.registry.remove(kindOrPoint, name);
    }
    list(kindOrPoint) {
        if (kindOrPoint.includes('.'))
            return this.middleware.list(kindOrPoint);
        return this.registry.list(kindOrPoint);
    }
    snapshot() {
        return this.registry.snapshot();
    }
    resolve(kind, name) {
        return this.registry.resolve(kind, name);
    }
    run(point, contract, state, turnId, payload) {
        const context = { point, contract, state, turnId, payload };
        return this.middleware.run(point, context);
    }
    async runGlobalHooks(method, payload) {
        const errors = [];
        for (const item of this.registry.list('globalHook')) {
            try {
                const hook = this.registry.resolve('globalHook', item.name);
                const fn = hook[method];
                if (fn) {
                    if (method === 'onContractUpgrade') {
                        const versions = payload;
                        await fn.call(hook, versions.from, versions.to);
                    }
                    else {
...
/** CES 动态正文注入槽位（只放 L3 / IN_CHAT，不污染稳定前缀）。 */
const EJS_PROMPT_KEY = 'nlkaleido:ejs:active';
/**
...
/** 异步任务完成时聊天已经变化：旧结果必须丢弃，绝不能写进新聊天。 */
class StaleChatSessionError extends Error {
    constructor() {
        super('聊天已切换，已丢弃旧聊天的异步结果');
        this.name = 'StaleChatSessionError';
    }
}
/** 只合并普通对象，数组与标量按字段整体替换；用于固定跨卡 global 字段空间。 */
function mergeGlobalFieldTree(base, incoming) {
    const out = { ...base };
    for (const [key, value] of Object.entries(incoming)) {
        const current = out[key];
        if (value && typeof value === 'object' && !Array.isArray(value)
            && current && typeof current === 'object' && !Array.isArray(current)) {
            out[key] = mergeGlobalFieldTree(current, value);
        }
        else {
            out[key] = value;
        }
...
class KaleidoStAdapter {
    ...
    adapter = {
        ...
        dicePresets: [],
        diceHistory: [],
        config: minimalConfigFallback(),
        configSnapshot: null,
        lastConfigReport: null,
        ejsActiveEntries: [],
        ejsLog: [],
        ejsQueuedEntries: [],
        sideEffectTriggeredAt: {},
    };
    compat = null;
    ...
    constructor(globals, options = {}) {
        ...
        const storage = createStorageRouter(globals);
        this.extensions = new ExtensionRuntime([storage.chat, storage.global], storage.unified);
    }
    /** ST 全局面（面板/桥经此访问 eventSource 等，§6.2 单向数据流） */
    get stGlobals() {
        return this.globals;
    }
    /** M13 真正按需加载：契约未启用记忆时不会请求 memory chunk。 */
    ...
    /** M14 配置中心首次打开或配置动作发生时才加载完整决策/探测实现。 */
    ...
    currentChatKey() {
        const context = this.globals.getContext();
        if (context.chatId !== null && context.chatId !== undefined)
            return `chat:${String(context.chatId)}`;
        return `fallback:${String(context.groupId ?? '')}:${String(context.characterId ?? '')}:no-chat`;
    }
    isSessionCurrent(expectedKey, expectedState) {
        return this.currentChatKey() === expectedKey
            && (this.adapter.state === null || this.adapter.state === expectedState);
    }
    // ============================================================
    // 初始化（§12 时序 A）
    // ============================================================
    init() {
        ...
        eventSource.on(eventTypes.GENERATION_ENDED, async () => {
            try {
                ...
                if (error instanceof StaleChatSessionError)
                    return;
                const message = error instanceof Error ? error.message : String(error);
                console.error('[NLKaleido] 回合更新失败：', error);
                this.onRuntimeError?.(`回合更新失败：${message}`);
            }
        });
        ...
        eventSource.on(eventTypes.GENERATION_STARTED, async () => {
            ...
            try {
                await this.prepareEjsInjection();
            }
            catch (error) {
                const message = error instanceof Error ? error.message : String(error);
                this.clearEjsInjection(false);
                console.error('[NLKaleido] CES 正文注入失败：', error);
                this.onRuntimeError?.(`CES 正文注入失败：${message}`);
            }
        });
        ...
        if (typeof eventTypes.CHAT_COMPLETION_SETTINGS_READY === 'string') {
            eventSource.on(eventTypes.CHAT_COMPLETION_SETTINGS_READY, async (generateData) => {
                await this.onSettingsReady(generateData);
            });
        }
        return this.compat;
    }
    ...
    async onGenerationEnded() {
        // 真实 ST 的停止按钮顺序是 hideStopButton() → GENERATION_ENDED → GENERATION_STOPPED。
        // ENDED 监听先让出一个 microtask，允许同一调用栈里的 STOPPED 把中止标记置上。
        await Promise.resolve();
        // 正文已经完成，立即撤掉本轮动态槽位，防止后续 quiet/变量请求意外继承。
        this.clearEjsPrompt();
        if (!this.adapter.contract || !this.adapter.state)
            return;
        const contract = this.adapter.contract;
        const state = this.adapter.state;
        const memoryStore = this.adapter.memoryStore;
        ...
        if (this.adapter.abortedThisGeneration)
            return;
        const turnId = state.meta.lastTurnId + 1;
        ...
        const boundary = checkRunBoundary(contract, state, sources);
        if (boundary.triggered) {
            await this.extensions.runGlobalHooks('onRunEnd', { runId: state.meta.runId, contractId: contract.id });
            beginNewRun(state, contract, state.meta.runId + 1);
            this.resetChatMemory(); // §16.1：chat 层记忆随周目重置，run/global 层保留
            this.resetChatPlot(); // §22.7：世界推演状态随周目重开
            await this.persistState(state, sessionKey);
            await this.extensions.runGlobalHooks('onRunStart', { runId: state.meta.runId, contractId: contract.id });
            this.onRunChanged?.(state.meta.runId, boundary.message);
        }
        ...
        if (contract.achievements?.length) {
            ...
            const unlocked = checkAchievements(contract, state, merged);
            if (unlocked.length) {
     ...
     */
    async prepareEjsInjection() {
        const contract = this.adapter.contract;
        const state = this.adapter.state;
        const context = this.globals.getContext();
        if (!contract || !state || (!contract.ejs?.length && !this.adapter.ejsQueuedEntries.length)) {
            this.clearEjsInjection(true);
            return;
        }
        if (typeof context.setExtensionPrompt !== 'function' || typeof context.loadWorldInfo !== 'function') {
            throw new Error('当前酒馆版本未暴露 loadWorldInfo / setExtensionPrompt');
        }
        const sessionKey = this.currentChatKey();
        const chat = this.globals.getChat();
        const result = evaluateEjs(contract, state, {
            recent_story: this.extractRecentStory(chat).map((m) => m.content).join('\n'),
            user_input: this.extractUserInput(chat),
        });
        const refs = [...(contract.ejs ?? []).flatMap((rule) => rule.entries)
                .filter((ref) => result.active.has(entryKey(ref))), ...this.adapter.ejsQueuedEntries]
            .filter((ref, index, all) => all.findIndex((x) => entryKey(x) === entryKey(ref)) === index);
        const worlds = new Map();
        await Promise.all([...new Set(refs.map((ref) => ref.world))].map(async (world) => {
            worlds.set(world, await context.loadWorldInfo(world));
        }));
        if (!this.isSessionCurrent(sessionKey, state))
            throw new StaleChatSessionError();
        const loaded = [];
        for (const ref of refs) {
            const worldData = worlds.get(ref.world);
            const entries = Array.isArray(worldData?.entries) ? worldData.entries : Object.values(worldData?.entries ?? {});
            const match = entries.find((raw) => {
                const entry = raw;
                return entry.comment === ref.name || entry.name === ref.name
                    || (Array.isArray(entry.key) && entry.key.includes(ref.name));
            });
            const content = String(match?.content ?? match?.text ?? match?.entry ?? '').trim();
            if (content)
                loaded.push({ key: entryKey(ref), content });
        }
        const injection = loaded.length
            ? `<NLKaleido_CES>\n${loaded.map((item) => `## ${item.key}\n${item.content}`).join('\n\n')}\n</NLKaleido_CES>`
            : '';
        context.setExtensionPrompt(EJS_PROMPT_KEY, injection, 1, 0, false, 0);
        this.adapter.ejsActiveEntries = loaded.map((item) => item.key);
        this.adapter.ejsLog = result.log;
        this.adapter.ejsQueuedEntries = [];
        this.onEjsChanged?.();
    }
    /** 清理 CES；resetState=true 时连面板状态一起重置（切聊天/无规则）。 */
    clearEjsInjection(resetState = true) {
        this.clearEjsPrompt();
        if (resetState) {
            this.adapter.ejsActiveEntries = [];
            this.adapter.ejsLog = [];
            this.adapter.ejsQueuedEntries = [];
            this.onEjsChanged?.();
        }
    ...
    clearEjsPrompt() {
        this.globals.getContext().setExtensionPrompt?.(EJS_PROMPT_KEY, '', 1, 0, false, 0);
    }
    /** 执行白名单 SideEffect 动作；判定在 Core 完成，IO 明确停留在适配层。 */
    async executeSideEffects(contract, state, sources) {
        const collected = collectSideEffectIntents(contract, state, sources, this.adapter.sideEffectTriggeredAt, Date.now());
        this.adapter.sideEffectTriggeredAt = collected.triggeredAt;
        for (const error of collected.errors)
            this.onRuntimeError?.(`SideEffect：${error}`);
        const context = this.globals.getContext();
        for (const intent of collected.intents) {
            try {
                if (intent.action === 'setStVar') {
                    const scope = intent.payload.scope === 'global' ? 'global' : 'local';
                    const name = String(intent.payload.name ?? '');
                    if (!name)
                        throw new Error('setStVar 缺少 payload.name');
                    const setter = context.variables?.[scope]?.set;
                    if (!setter)
                        throw new Error(`酒馆变量桥不支持 ${scope}.set`);
                    await setter(name, intent.payload.value);
                }
                else if (intent.action === 'emitEvent') {
                    const event = String(intent.payload.event ?? `nlkaleido:side_effect:${intent.effectId}`);
                    await context.eventSource.emit(event, intent.payload.data ?? intent.payload);
                }
                else if (intent.action === 'queueWorldInfo') {
                    const world = String(intent.payload.world ?? '');
                    const name = String(intent.payload.name ?? '');
                    if (!world || !name)
                        throw new Error('queueWorldInfo 缺少 world/name');
                    this.adapter.ejsQueuedEntries.push({ world, name });
                }
                else if (intent.action === 'callHook') {
                    const name = String(intent.payload.name ?? '');
                    const hook = this.extensions.resolve('globalHook', name);
                    if (typeof hook.handle !== 'function')
                        throw new Error(`GlobalHook '${name}' 未实现 handle`);
                    await hook.handle(intent.payload.data ?? intent.payload);
                }
                else {
                    await context.eventSource.emit('nlkaleido:notification', {
                        effectId: intent.effectId,
                        level: intent.payload.level ?? 'info',
                        message: String(intent.payload.message ?? ''),
                    });
                }
            }
            catch (error) {
                this.onRuntimeError?.(`SideEffect '${intent.effectId}/${intent.action}'：${error instanceof Error ? error.message : String(error)}`);
            }
        }
    }
    /** 任意状态写入口（Agent/手动/pending 接受）共用的 OP-4/ST-1 派生与副作用阶段。 */
    async recomputeDerivedAndEffects(contract, state, turnId, sources = {}) {
        const derivedChanges = evaluateDerivedFields(contract, state, (name) => this.extensions.resolve('derivedFn', name));
        for (const change of derivedChanges) {
            state.revision = {
                seq: state.revision.seq + 1,
                hash: hash64(`${change.path}|${JSON.stringify(change.newValue)}|${state.revision.seq + 1}`),
                updatedAt: Date.now(),
            };
            state.changelog.push({
                seq: state.revision.seq, turnId, path: change.path,
                op: { op: 'replace', path: change.path, value: change.newValue, confidence: 'high', rationale: `派生字段 ${change.name}` },
                old: change.oldValue, new: change.newValue, confidence: 'high', source: 'derived',
            });
        }
        await this.executeSideEffects(contract, state, sources);
    }
    // ============================================================
        ...
        try {
            ...
            try {
                ...
                if (parsed && typeof parsed === 'object' && !Array.isArray(parsed)) {
                    ...
                    if (ledgerLines.length)
                        this.appendPlotLedger(ledgerLines);
                    await this.persistPlotState(sessionKey, state);
                    this.onPlotChanged?.();
                }
            }
            finally {
    ...
    async persistPlotState(expectedChatKey = this.currentChatKey(), expectedState = this.adapter.state) {
        const blob = {
        ...
        return this.commit.enqueue(async () => {
            if (expectedState && !this.isSessionCurrent(expectedChatKey, expectedState))
                throw new StaleChatSessionError();
    ...
    async loadConfig() {
        ...
        this.adapter.config = config ?? configApi.tierDefaults('minimal');
        // 默认值、损坏恢复值或迁移结果立即写回，避免每次刷新重复恢复/告警。
        if (stored === undefined || stored === null || config === null || migrated) {
            store[STORE_KEY_CONFIG] = this.adapter.config;
            this.globals.saveSettings();
        }
        const backup = store[STORE_KEY_CONFIG_BACKUP];
    ...
    async applyDestructiveConfig(config, reason) {
        const configApi = await this.ensureConfigModule();
        const snapshot = configApi.takeSnapshot(this.adapter.config, reason);
    ...
    async factoryResetConfig() {
        const configApi = await this.ensureConfigModule();
        await this.saveConfig(configApi.tierDefaults('minimal'));
        return this.adapter.config;
    }
    ...
    async applyTier(preferred, env = {}) {
        ...
        try {
            ...
            const checks = configApi.runSelfCheck(this.adapter.config, {
                storageWriteOk: true,
    ...
    async archiveChangelog(contract) {
        ...
        try {
            ...
            if (!this.isSessionCurrent(sessionKey, state))
                return;
            if (summary) {
                const committed = memoryApi.commitArchive(state.changelog, selection.batch, summary);
                state.changelog = committed.entries;
                this.adapter.memoryStore.atoms.push(memoryApi.makeAtom({
                    content: `[归档] ${summary}`,
                    type: 'factual',
                    importance: 0.8,
                    confidence: 0.8,
    ...
    searchMemoryText(query, topK = 5) {
        if (!this.memoryModule)
            return '（记忆模块尚未加载）';
        const atoms = this.memoryModule.searchMemory(this.adapter.memoryStore, query, { scope: 'chat', activeOnly: true, topK });
        if (!atoms.length)
            return '（无相关长期记忆）';
    ...
    async generateContractDraft(description) {
        ...
        this.pendingJsonSchema = {
            ...
            description: 'NLKaleido 完整契约 JSON',
    ...
    async onSettingsReady(generateData) {
        ...
        if (contract && state) {
            await this.extensions.run('request.settingsReady', contract, state, state.meta.lastTurnId + 1, generateData);
        }
    }
    // ============================================================
    ...
    /**
     * 显式付费诊断：向酒馆当前 provider 发送相同稳定前缀 + 不同短尾部，读取真实 usage。
     * 不读取聊天、不修改契约/状态、不写盘；默认绝不运行，调用者必须传 confirmCost:true。
     */
    async probeProviderCache(options) {
        ...
        const stablePrefix = [
            ...
            '这是一段只用于验证前缀缓存的稳定文本；请忽略内容并仅回复 OK。',
            '稳定前缀不得被模型解释为角色扮演指令。'.repeat(96),
        ].join('\n');
        const prefixFingerprint = hash64(stablePrefix);
        const captures = [];
        this.providerProbeRunning = true;
        try {
            for (let index = 0; index < rounds; index += 1) {
                ...
                captures.push({
                    ...
                    usageAvailable: metrics !== null,
                    promptTokens: metrics?.promptTokens ?? 0,
                    completionTokens: metrics?.completionTokens ?? 0,
                    hitTokens: hitMiss.hitTokens,
                    missTokens: hitMiss.missTokens,
                    usage: metrics?.usage ?? null,
                });
            }
        }
        finally {
            this.providerProbeRunning = false;
        }
        const warm = captures.slice(1);
        const warmHitTokens = warm.reduce((sum, item) => sum + item.hitTokens, 0);
        const warmMissTokens = warm.reduce((sum, item) => sum + item.missTokens, 0);
        return {
            schemaVersion: 1,
            ...
            summary: {
                stablePrefix: true,
                usageAvailable: captures.every((item) => item.usageAvailable),
                warmHitTokens,
                warmMissTokens,
                phitAfterWarmup: warmHitTokens + warmMissTokens > 0 ? warmHitTokens / (warmHitTokens + warmMissTokens) : 0,
                cacheHitObserved: warmHitTokens > 0,
            },
        };
    }
    // ============================================================
    ...
    async toolGetState() {
        ...
        if (!contract || !state)
            return '{}';
        const backend = this.extensions.resolve('renderBackend');
        const view = buildStateTableView(contract, state, 'full');
        await this.extensions.run('render.stateTable', contract, state, state.meta.lastTurnId, view);
        return backend.render(view, { turnId: state.meta.lastTurnId, budget: contract.guardrails.maxStatusTokens });
    }
    async toolApplyPatch(ops) {
        ...
        await this.recomputeDerivedAndEffects(contract, state, state.meta.lastTurnId + 1);
    ...
    async requestMultiStepUpdate() {
        ...
        const schema = {
            ...
            description: '选择下一工具动作：apply_patch 修改状态，get_state 刷新状态，done 结束。',
            value: {
                type: 'object',
                properties: {
                    action: { type: 'string', enum: ['get_state', 'apply_patch', 'done'] },
                    rationale: { type: 'string' },
                    ops: {
                        type: 'array',
                        items: {
                            ...
                            properties: {
                                op: { type: 'string', enum: ['replace', 'delta', 'add', 'remove', 'move'] },
                                path: { type: 'string' }, value: {}, from: { type: 'string' },
        ...
        const result = await runMultiStepAgent({
            ...
            decide: async ({ step, stateView, feedback, history }) => {
                if (!this.isSessionCurrent(sessionKey, state))
                    throw new StaleChatSessionError();
                const prompt = [
                    { role: 'system', name: VREQ_MARKER, content: '' },
                    {
                        ...
                        content: [
                            ...
                            `当前步骤：${step}`,
                            `工具反馈：${feedback}`,
                            `近期工具历史：${JSON.stringify(history)}`,
                            `最新状态：\n${stateView}`,
                            `最近剧情：\n${this.extractRecentStory(this.globals.getChat()).map((m) => `${m.role}: ${m.content}`).join('\n').slice(-5e3)}`,
                        ].join('\n\n'),
                    },
                ];
                ...
                try {
                    ...
                    if (action !== 'get_state' && action !== 'apply_patch' && action !== 'done') {
                        throw new Error(`多步 Agent 返回了非法 action：${String(action)}`);
                    }
                    const usage = data?.usage;
                    const tokens = typeof usage?.completion_tokens === 'number' ? usage.completion_tokens : undefined;
                    if (action === 'apply_patch') {
                        return { action, ops: Array.isArray(parsed?.ops) ? parsed.ops : [], rationale: String(parsed?.rationale ?? ''), tokens };
                    }
                    return { action, rationale: String(parsed?.rationale ?? ''), tokens };
                }
                finally {
                    this.pendingJsonSchema = null;
                }
    ...
    persistState(state, expectedChatKey = this.currentChatKey()) {
        return this.commit.enqueue(async () => {
            try {
                if (!this.isSessionCurrent(expectedChatKey, state))
                    throw new StaleChatSessionError();
                const contract = this.adapter.contract;
                if (contract)
                    await this.extensions.run('persist.before', contract, state, state.meta.lastTurnId, { expectedChatKey });
                ...
                // 契约与状态必须同聊天原子落盘。否则作者保存后只在当前内存生效，刷新即归零。
                const chatState = { ...state, stat_data: layers.chat, contract };
                const storageProvider = this.extensions.resolve('storageProvider');
                if (storageProvider.id !== 'builtin:storageProvider') {
                    const storageMeta = { reason: 'persistState', seq: state.revision.seq };
                    ...
                    if (contract) {
                        ...
                        const globalKey = `${STORE_KEY_GLOBAL_PREFIX}global.fields`;
                        const currentGlobal = await storageProvider.load(globalKey);
                        await storageProvider.save(globalKey, mergeGlobalFieldTree(currentGlobal && typeof currentGlobal === 'object' ? currentGlobal : {}, layers.global), storageMeta);
                    }
                    await storageProvider.save(`${STORE_KEY_GLOBAL_PREFIX}revision`, { seq: state.revision.seq, hash: state.revision.hash, contractVersion: state.contractVersion }, storageMeta);
                    if (contract)
                        await this.extensions.run('persist.after', contract, state, state.meta.lastTurnId, { expectedChatKey });
                ...
                if (contract) {
                    ...
                    // global 字段使用固定跨卡命名空间；同一路径在不同角色卡/聊天中共享。
                    globalStore[`${STORE_KEY_GLOBAL_PREFIX}global.fields`] = mergeGlobalFieldTree(globalFields, layers.global);
                }
                ...
                variables.global = globalStore;
                ext.variables = variables;
                await this.globals.saveSettings();
                if (contract)
                    await this.extensions.run('persist.after', contract, state, state.meta.lastTurnId, { expectedChatKey });
                this.commit.recordError(null);
            }
    ...
    /** 可替换 StorageProvider 的异步读取入口；默认实现仍保持本地同步快路径。 */
    async loadStateAsync() {
        const storageProvider = this.extensions.resolve('storageProvider');
        if (storageProvider.id === 'builtin:storageProvider')
            return this.loadState();
        const stored = await storageProvider.load(STORE_KEY_CHAT);
    ...
    async hydrateMemoryTables(contract) {
        ...
        const tables = declared.map((def) => {
            ...
            const table = old ?? memoryApi.createMemoryTable(def.id, def.name, def.columns, def.floorScoped);
    ...
    hydratePersistLayers(state, contract) {
        ...
        let globalFields = (globalStore[`${STORE_KEY_GLOBAL_PREFIX}global.fields`] ?? {});
        // v0.4.0 及更早错误地按 contract.id 隔离 global。检测到旧槽且当前固定空间缺字段时，
        // 原地迁移一次并删除旧槽，避免每次加载反复覆盖新值。
        const legacy = globalFields[contract.id];
        const globalPaths = Object.values(contract.updateRules).filter((field) => (field.persist ?? 'chat') === 'global').map((field) => field.path);
        if (legacy && typeof legacy === 'object' && !Array.isArray(legacy)
            && globalPaths.some((path) => getAtPath(globalFields, path) === undefined && getAtPath(legacy, path) !== undefined)) {
            const migrated = mergeGlobalFieldTree(globalFields, legacy);
            delete migrated[contract.id];
            globalFields = migrated;
            globalStore[`${STORE_KEY_GLOBAL_PREFIX}global.fields`] = migrated;
            this.globals.saveSettings();
        }
        const globalLayer = globalFields;
...
const NL_EVENTS = Object.freeze({
    ...
    RUNTIME_ERROR: 'nlkaleido:runtime_error',
});
class KaleidoStateBridge {
    globals;
    adapter;
    pendingTasks = new Set();
    /** 公共前端写入严格串行，避免两个 UI 组件同时 set 时交叉覆盖审计与落盘。 */
    variableWriteQueue = Promise.resolve();
    ...
    notifyEjsChanged() {
        this.emit(NL_EVENTS.EJS_CHANGED, {
            activeEntries: this.adapter.adapter.ejsActiveEntries,
            log: this.adapter.adapter.ejsLog,
        });
    }
    notifyRuntimeError(message) {
        this.emit(NL_EVENTS.RUNTIME_ERROR, { message, at: Date.now() });
    }
    background(promise, onSuccess) {
        let task;
        task = promise
            .then((value) => { onSuccess?.(value); })
            .catch((error) => {
            this.notifyRuntimeError(error instanceof Error ? error.message : String(error));
        })
            .finally(() => { this.pendingTasks.delete(task); });
        this.pendingTasks.add(task);
    }
    /** 等待由 dispatch 派生的异步校验、落盘和事件回调全部收敛。 */
    async whenIdle() {
        while (this.pendingTasks.size > 0) {
            await Promise.all([...this.pendingTasks]);
        }
    }
    /** 前端公开读取：只返回契约声明变量，不暴露内部 meta/changelog。 */
    getVariable(path) {
        const contract = this.adapter.adapter.contract;
        const state = this.adapter.adapter.state;
        if (!contract?.updateRules[path] || !state)
            return undefined;
        return structuredClone(getAtPath(state.stat_data, path));
    }
    /** 前端公开变量清单：可用于自动生成状态组件、表单和调试面板。 */
    listVariables() {
        const contract = this.adapter.adapter.contract;
        if (!contract)
            return [];
        return Object.values(contract.updateRules).map((field) => ({
            path: field.path,
            type: field.type,
    ...
    /**
     * 前端正式写入口：校验声明、类型、权限、不变量；await 完成时已重算派生值并真实落盘。
     */
    async setVariables(values, source = 'frontend') {
        const task = this.variableWriteQueue.then(async () => {
            const started = this.prepareVariableWrite(values, source);
            if (!started.ok || !started.completion)
                return started;
            await started.completion;
            return { ok: true, changed: started.changed, revision: this.adapter.adapter.state?.revision.seq };
        });
        this.variableWriteQueue = task.then(() => undefined, () => undefined);
        return task;
    }
    async setVariable(path, value, source = 'frontend') {
        return this.setVariables({ [path]: value }, source);
    }
    /** 状态订阅：返回取消订阅函数；事件载荷始终为最新公开变量清单。 */
    subscribeVariables(listener) {
        const eventSource = this.globals.getContext().eventSource;
        const handler = () => listener(this.listVariables());
        eventSource.on(NL_EVENTS.STATUS_CHANGED, handler);
        return () => { eventSource.off?.(NL_EVENTS.STATUS_CHANGED, handler); };
    }
    prepareVariableWrite(values, source) {
        const contract = this.adapter.adapter.contract;
        const state = this.adapter.adapter.state;
        if (!contract || !state)
            return { ok: false, error: '变量系统尚未加载完成' };
        const entries = Object.entries(values);
        if (!entries.length)
            return { ok: false, error: '没有要写入的变量' };
        ...
        applyOps(state, validated.applied, state.meta.lastTurnId, { source, dedupe: false });
        const changed = validated.applied.map((op) => op.path);
        const completion = this.adapter.recomputeDerivedAndEffects(contract, state, state.meta.lastTurnId)
            .then(() => this.adapter.persistState(state))
            .then(() => { this.notifyStatusChanged(state); })
            .catch((error) => {
            // Promise 失败时恢复当前内存态，调用者可放心重试，不会留下“界面看似改了但未保存”的半状态。
            for (const key of Object.keys(state))
                delete state[key];
            Object.assign(state, before);
            throw error;
        });
        return { ok: true, changed, revision: state.revision.seq, completion };
    }
    ...
    dispatch(payload) {
        ...
        switch (payload.action) {
            case 'setVariable': {
                ...
                const started = this.prepareVariableWrite({ [path]: payload.value }, 'manual');
                if (!started.ok || !started.completion)
                    return started;
                this.background(started.completion);
                return { ok: true, result: { changed: started.changed ?? [], revision: started.revision ?? state.revision.seq } };
            }
            case 'editContract': {
                try {
                    const previousVersion = this.adapter.adapter.contract?.version ?? state.contractVersion;
                    const raw = payload.contract;
                    const parsed = parseContract(raw);
                    const resolved = normalizeContract(parsed);
                    resolved.version = Math.max(resolved.version, previousVersion + 1);
                    const errors = [
                        ...validateContract(resolved, { registry: this.adapter.extensions.registry }),
                        ...resolveContractMiddleware(resolved.middleware, this.adapter.extensions.middleware),
                    ];
                    if (errors.length)
                        return { ok: false, error: errors.join('；') };
                    this.adapter.adapter.contract = resolved;
                    state.stat_data = reconcileData(resolved, state.stat_data);
                    state.contractVersion = resolved.version;
                    state.meta.lastContractVersion = resolved.version;
                    this.background(this.adapter.extensions.runGlobalHooks('onContractUpgrade', { from: previousVersion, to: resolved.version }), (hookErrors) => hookErrors.forEach((error) => this.notifyRuntimeError(`GlobalHook：${error}`)));
                    this.background(this.adapter.recomputeDerivedAndEffects(resolved, state, state.meta.lastTurnId)
                        .then(() => this.adapter.persistState(state)), () => {
            ...
            case 'rollback': {
                ...
                if (!result.ok)
                    return result;
                const contract = this.adapter.adapter.contract;
                this.background((contract ? this.adapter.recomputeDerivedAndEffects(contract, state, state.meta.lastTurnId) : Promise.resolve())
                    .then(() => this.adapter.persistState(state)), () => this.notifyStatusChanged(state));
                return { ok: true };
            }
            case 'beginNewRun': {
                const contract = this.adapter.adapter.contract;
                ...
                this.background(this.adapter.extensions.runGlobalHooks('onRunEnd', { runId: state.meta.runId, contractId: contract.id }));
                ...
                this.background(this.adapter.persistState(state), () => {
                    this.notifyStatusChanged(state);
                    this.notifyRunChanged(newRunId);
                    this.background(this.adapter.extensions.runGlobalHooks('onRunStart', { runId: newRunId, contractId: contract.id }));
                });
                return { ok: true };
            }
            ...
            case 'plotTerminal': {
                ...
                this.background(this.adapter.persistPlotState(), () => this.notifyPlotChanged());
                return { ok: true };
            }
            ...
            case 'memoryDelete': {
                ...
                this.background(this.adapter.persistState(state), () => this.notifyMemoryChanged());
                return { ok: true };
            }
            ...
            case 'configApplyTier': {
                ...
                this.background(this.adapter.applyTier(tier), (result) => {
            ...
            case 'configRollback':
                this.background(this.adapter.rollbackConfig(), () => this.notifyConfigChanged());
                return { ok: true };
            case 'configFactoryReset':
                if (payload.confirmed !== true)
                    return { ok: false, error: '恢复出厂为破坏性操作：需二次确认（confirmed:true）' };
                this.background(this.adapter.factoryResetConfig(), () => this.notifyConfigChanged());
                return { ok: true };
            case 'configSave':
                this.background(this.adapter.saveConfig(payload.config), () => this.notifyConfigChanged());
                return { ok: true };
            default:
...
function cloneContract(contract) {
    return cloneData(contract);
}
/** 契约是纯数据；逐项读取可安全剥离 Vue Proxy，避免 structuredClone(DataCloneError)。 */
...
function uniqueFieldPath(contract, preferred) {
    if (!contract.updateRules[preferred])
        return preferred;
    let index = 2;
    while (contract.updateRules[`${preferred}${index}`])
        index += 1;
    return `${preferred}${index}`;
}
function upsertField(contract, previousPath, field) {
    const next = cloneContract(contract);
    if (previousPath && previousPath !== field.path)
        delete next.updateRules[previousPath];
    next.updateRules[field.path] = cloneData(field);
    return next;
}
function removeField(contract, path) {
    const next = cloneContract(contract);
    delete next.updateRules[path];
    next.displayRules = next.displayRules.filter((rule) => rule.path !== path);
    for (const field of Object.values(next.updateRules)) {
        field.dependencies = field.dependencies?.filter((dependency) => dependency !== path);
    }
    return next;
}
function duplicateField(contract, path) {
    const source = contract.updateRules[path];
    if (!source)
        throw new Error(`字段不存在：${path}`);
    const nextPath = uniqueFieldPath(contract, `${path}_副本`);
...
function fieldGroups(contract) {
    ...
    for (const field of Object.values(contract.updateRules)) {
        const group = field.path.split('.')[0] || '未分组';
...
/** 面板只需要展示衰减值；保留轻量公式，避免默认路径静态加载完整记忆引擎。 */
function memoryDecayScore(decay, ttlDays, daysSince) {
    if (daysSince <= 0)
        return 1;
    if (decay === 'linear')
...
const PlayerView = defineComponent({
    ...
    setup() {
        ...
        const groups = () => {
            ...
            if (!contract)
                return [];
            return fieldGroups(contract).map((group) => ({
                name: group.name,
        ...
        return () => {
            ...
            const achievements = contract ? adapter.readAchievements(contract.id) : [];
            const unlocked = achievements.filter((item) => item.unlocked).length;
            const visibleGroups = groups();
            if (!contract || !state || !visibleGroups.length)
                return h('div', { class: 'nlk-empty-state' }, [
                    h('div', { class: 'nlk-empty-icon' }, '◇'), h('h2', null, '这张卡还没有状态面板'),
                    h('p', null, '从字段表单创建第一个变量，或用一句话让模型生成契约草稿。'),
                    h('button', { class: 'nlk-btn nlk-btn-primary', onClick: () => { store.roleMode = 'author'; store.activeTab = 'author'; store.authorMode = true; } }, '我是作者，开始设置变量'),
                ]);
            return h('div', { class: 'nlk-dashboard' }, [
                ...
                h('section', { class: 'nlk-kpis' }, [
                    h('div', { class: 'nlk-kpi' }, [h('strong', null, String(store.lastTurnId)), h('span', null, '当前轮次')]),
                    h('div', { class: 'nlk-kpi' }, [h('strong', null, String(state.meta.runId)), h('span', null, '当前周目')]),
                    h('div', { class: store.pending.length ? 'nlk-kpi nlk-kpi-warn' : 'nlk-kpi' }, [h('strong', null, String(store.pending.length)), h('span', null, '待复核')]),
                    h('div', { class: 'nlk-kpi' }, [h('strong', null, `${unlocked}/${contract.achievements?.length ?? 0}`), h('span', null, '成就')]),
                ]),
                adapter.adapter.ejsActiveEntries.length ? h('div', { class: 'nlk-callout' }, `本轮启用的附加设定：${adapter.adapter.ejsActiveEntries.join('、')}`) : null,
                h('section', { class: 'nlk-status-grid' }, visibleGroups.map((group) => h('article', { class: 'nlk-status-card' }, [
                ...
                contract.achievements?.length ? h('section', { class: 'nlk-card nlk-achievements' }, [
                    h('div', { class: 'nlk-card-title' }, [h('span', null, '成就墙'), h('small', null, `${unlocked}/${contract.achievements.length}`)]),
                    h('div', { class: 'nlk-achievement-grid' }, contract.achievements.map((declared) => {
                        const saved = achievements.find((item) => item.id === declared.id);
                        const isUnlocked = saved?.unlocked === true;
                        return h('div', { class: isUnlocked ? 'nlk-achievement nlk-achievement-on' : 'nlk-achievement' }, [h('span', null, declared.icon ?? (isUnlocked ? '◆' : '◇')), h('div', null, [h('strong', null, declared.hidden && !isUnlocked ? '隐藏成就' : declared.name), h('small', null, declared.desc ?? `${declared.progressVar} / ${declared.target}`)])]);
                    })),
                ]) : null,
            ]);
        };
...
/** 作者工作台：表单是主入口，JSON 只放高级区；支持自然语言生成草稿。 */
    ...
    setup() {
        ...
        const loadDraft = (text) => {
            try {
                ...
                draft.value = cloneContract(parsed);
                selectedPath.value = selectedPath.value && parsed.updateRules[selectedPath.value]
                    ? selectedPath.value : Object.keys(parsed.updateRules)[0] ?? '';
            }
            catch {
                // 正在高级编辑半截 JSON 时不覆盖上一个合法草稿。
            }
        };
        watch(() => store.contractText, (text) => { if (!dirty.value)
            loadDraft(text); }, { immediate: true });
        const tell = (message, kind = 'info') => {
        ...
        const patchField = (patch) => {
            if (!draft.value || !selectedPath.value)
                return;
            const current = draft.value.updateRules[selectedPath.value];
            if (!current)
                return;
            const next = { ...current, ...patch };
            draft.value = upsertField(draft.value, selectedPath.value, next);
        ...
        const addField = () => {
            if (!draft.value)
                return;
            const path = uniqueFieldPath(draft.value, '新分组.新字段');
            draft.value = upsertField(draft.value, null, createField(path));
        ...
        const copyField = () => {
            if (!draft.value || !selectedPath.value)
                return;
            const copied = duplicateField(draft.value, selectedPath.value);
            draft.value = copied.contract;
            selectedPath.value = copied.path;
            changed();
        };
        const deleteField = () => {
            if (!draft.value || !selectedPath.value)
                return;
            draft.value = removeField(draft.value, selectedPath.value);
        ...
        const removeCes = (index) => {
            if (!draft.value)
                return;
            draft.value.ejs = (draft.value.ejs ?? []).filter((_, i) => i !== index);
            changed();
        };
        ...
        const editAdvanced = (text) => {
            ...
            dirty.value = true;
            try {
                draft.value = JSON.parse(text);
                tell('JSON 语法有效；保存时还会做完整契约校验。', 'ok');
            }
            catch (error) {
                tell(`JSON 尚未完整：${error instanceof Error ? error.message : String(error)}`, 'error');
            }
        };
        const input = (label, value, onInput, attrs = {}, help = '') => h('label', { class: 'nlk-field' }, [
            h('span', { class: 'nlk-label' }, label),
            help ? h('small', { class: 'nlk-help' }, help) : null,
            h('input', { class: 'nlk-input', value, onInput: (e) => onInput(e.target.value), ...attrs }),
        ]);
        const select = (label, value, values, onChange, help = '') => h('label', { class: 'nlk-field' }, [
            h('span', { class: 'nlk-label' }, label),
            help ? h('small', { class: 'nlk-help' }, help) : null,
        ...
        return () => {
            ...
            if (section.value === 'fields') {
                content = h('div', { class: 'nlk-author-grid' }, [
                    h('aside', { class: 'nlk-field-list' }, [
                        h('div', { class: 'nlk-pane-title' }, ['要记录的变量', h('button', { class: 'nlk-icon-btn', onClick: addField, title: '新增变量' }, '+')]),
                        ...fieldGroups(contract).map((group) => h('div', { class: 'nlk-field-group' }, [
                            h('div', { class: 'nlk-group-name' }, group.name),
                            ...group.fields.map((item) => h('button', {
                                class: selectedPath.value === item.path ? 'nlk-field-item nlk-field-item-active' : 'nlk-field-item',
                                onClick: () => { selectedPath.value = item.path; },
                            }, [h('span', null, item.path.split('.').slice(1).join('.') || item.path), h('small', null, item.type)])),
                        ])),
                    ]),
                    field ? h('section', { class: 'nlk-editor-pane' }, [
                        h('div', { class: 'nlk-pane-title' }, [h('span', null, `正在设置：${field.path}`), h('div', { class: 'nlk-row nlk-row-tight' }, [
                                h('button', { class: 'nlk-btn nlk-btn-sm', onClick: copyField }, '复制'),
                                h('button', { class: 'nlk-btn nlk-btn-sm nlk-danger', onClick: deleteField }, '删除'),
                            ])]),
                    ...
                    ...(contract.ejs ?? []).map((rule, index) => {
                        ...
                        return h('article', { class: 'nlk-card' }, [
                            ...
                            condition ? h('div', { class: 'nlk-form-grid' }, [
                                input('看哪个变量', condition.var, (value) => patchCes(index, { condition: { ...condition, var: value } })),
                ...
                content = h('div', { class: 'nlk-stack' }, [
                    h('div', { class: 'nlk-callout nlk-callout-warn' }, '这里直接编辑底层配置，只适合熟悉 JSON 和 NLKaleido 数据结构的开发者。普通作者不需要使用；错误内容会在保存前被拦截。'),
                    h('textarea', { class: 'nlk-code-editor', rows: 24, value: json(contract), onInput: (e) => editAdvanced(e.target.value) }),
                ]);
            }
            return h('div', { class: 'nlk-section' }, [
                h('section', { class: 'nlk-author-hero' }, [
...
const PreviewView = defineComponent({
    ...
    setup() {
        ...
        return () => h('div', { class: 'nlk-section' }, [
            h('div', { class: 'nlk-callout' }, '这里预览变量会怎样提供给 AI。正常情况下使用“完整内容”；变量很多时可用摘要或仅变化内容来节省上下文。'),
            h('div', { class: 'nlk-row' }, [
                [['full', '完整内容'], ['summary', '简短摘要'], ['incremental', '仅本轮变化']].map(([m, label]) => h('button', {
                    key: m,
            ...
            h('div', { class: 'nlk-h3' }, '变量 AI 这一轮实际能看到的内容'),
    ...
    setup() {
        ...
        const run = async () => {
            ...
            try {
                ...
                const migration = await import('./chunks/migration-B1xpFSQl.js');
        ...
        return () => h('div', { class: 'nlk-section' }, [
            ...
            h('input', { class: 'nlk-input', type: 'file', accept: '.json,.txt', onChange: (event) => { void readUpload(event, worldbookText); } }),
            ...
            h('input', { class: 'nlk-input', type: 'file', accept: '.js,.ts,.txt', onChange: (event) => { void readUpload(event, scriptsText); } }),
    ...
    setup() {
        ...
        const factoryReset = () => {
            if (!confirmReset.value) {
                ...
                actionMessage.value = '这是破坏性操作。再次点击“确认恢复”才会执行，执行前自动备份。';
                return;
            }
            const result = dispatch({ action: 'configFactoryReset', confirmed: true });
    ...
    setup() {
        ...
        return () => h('section', { class: 'nlk-role-gate' }, [
            h('div', { class: 'nlk-role-intro' }, [
                h('div', { class: 'nlk-brand-mark nlk-role-mark' }, '◇'),
            ...
            h('div', { class: 'nlk-role-grid' }, [
                h('button', { class: 'nlk-role-card nlk-role-player', onClick: () => choose('player') }, [
                    h('span', { class: 'nlk-role-icon' }, '⌂'), h('strong', null, '我是玩家'),
                    h('span', null, '直接进入状态页面，查看角色属性、关系、背包、世界进度和待确认变化。'),
                    h('em', null, '进入玩家页面 →'),
                ]),
                h('button', { class: 'nlk-role-card nlk-role-author', onClick: () => choose('author') }, [
                    h('span', { class: 'nlk-role-icon' }, '✦'), h('strong', null, '我是作者'),
                    h('span', null, '用表单创建变量，说明 AI 什么时候更新，并决定玩家页面展示什么。'),
                    h('em', null, '进入作者向导 →'),
                ]),
            ]),
        ]);
...
const PanelRoot = defineComponent({
    ...
    setup() {
        const store = useNlStore();
        const tab = (name, icon, label) => h('button', {
            class: store.activeTab === name ? 'nlk-nav-item nlk-nav-active' : 'nlk-nav-item',
            onClick: () => {
        ...
        const page = () => store.activeTab === 'author'
            ? h('div', null, [h(ContractEditor), h('details', { class: 'nlk-technical' }, [h('summary', null, '高级检查工具（变量预览、修改记录、回滚）'), h(PreviewView), h(DebugPanel)])])
...
.nlkaleido-workbench.nlk-open { display: flex; }
.nlkaleido-workbench-card {
  display: flex; flex-direction: column; width: min(1120px, 96vw); height: min(820px, 92vh);
  overflow: hidden; border: 1px solid var(--SmartThemeBorderColor, rgba(255,255,255,.25));
  border-radius: 12px; background: var(--SmartThemeBlurTintColor, #202027);
  color: var(--SmartThemeBodyColor, #eee); box-shadow: 0 18px 70px rgba(0,0,0,.65);
}
.nlkaleido-workbench-header {
  display: flex; align-items: center; gap: 10px; padding: 10px 14px;
  border-bottom: 1px solid var(--SmartThemeBorderColor, rgba(255,255,255,.18));
  background: rgba(0,0,0,.22);
}
.nlkaleido-workbench-title { font-size: 16px; font-weight: 700; }
...
.nlk-role-gate {
  ...
  color: var(--SmartThemeBodyColor, #e8e8ee);
  font-family: Inter, ui-sans-serif, system-ui, -apple-system, "Segoe UI", sans-serif;
  background: radial-gradient(circle at 50% 12%, rgba(128,92,255,.16), transparent 42%);
}
...
.nlk-root {
  ...
  font-family: Inter, ui-sans-serif, system-ui, -apple-system, "Segoe UI", sans-serif;
  color: var(--SmartThemeBodyColor, #e8e8ee); font-size: 13px;
}
.nlkaleido-workbench-body { padding: 0; }
...
@media (max-width: 700px) {
  ...
  .nlkaleido-workbench-card { width: 100vw; height: 100dvh; border-radius: 0; border: 0; }
  .nlkaleido-workbench-header { padding: 8px 10px; }
  .nlkaleido-workbench-body { padding: 10px; }
  .nlk-role-gate { min-height: 100%; padding: 24px 14px; place-content: start center; }
  .nlk-role-grid { width: 100%; grid-template-columns: 1fr; gap: 10px; }
  .nlk-role-card { min-height: 155px; padding: 18px; }
  .nlk-root { display: block; }
 ...
 *
 * 主界面永远挂在 document.body 的独立工作台中，不依赖 ST 设置弹层是否已打开；
 * #extensions_settings2 / #extensions_settings 只承载一个入口。这样在 ST 调整设置 DOM、
...
function mountPanel(deps) {
    ...
    workbench.id = 'nlkaleido_workbench';
    workbench.className = 'nlkaleido-workbench';
    workbench.setAttribute('role', 'dialog');
    workbench.setAttribute('aria-modal', 'true');
    workbench.setAttribute('aria-hidden', 'true');
    workbench.setAttribute('aria-labelledby', 'nlkaleido_workbench_title');
    const card = document.createElement('div');
    card.className = 'nlkaleido-workbench-card';
    const header = document.createElement('div');
    header.className = 'nlkaleido-workbench-header';
    const title = document.createElement('div');
    title.id = 'nlkaleido_workbench_title';
    title.className = 'nlkaleido-workbench-title';
    title.textContent = 'NLKaleido 万花筒';
    const runtimeStatus = document.createElement('div');
    runtimeStatus.id = 'nlkaleido_runtime_status';
    runtimeStatus.className = 'nlkaleido-workbench-status';
    const closeButton = document.createElement('button');
    closeButton.type = 'button';
    closeButton.className = 'menu_button nlk-btn nlkaleido-workbench-close';
    closeButton.setAttribute('aria-label', '关闭万花筒工作台');
    closeButton.textContent = '×';
    const panelBody = document.createElement('div');
    panelBody.id = 'nlkaleido_panel_container';
    panelBody.className = 'nlkaleido-workbench-body';
    header.append(title, runtimeStatus, closeButton);
    card.append(header, panelBody);
    workbench.appendChild(card);
    document.body.appendChild(workbench);
    const openWorkbench = () => {
        workbench.classList.add('nlk-open');
        workbench.setAttribute('aria-hidden', 'false');
        closeButton.focus();
    };
    const closeWorkbench = () => {
        workbench.classList.remove('nlk-open');
        workbench.setAttribute('aria-hidden', 'true');
    };
    closeButton.addEventListener('click', closeWorkbench);
    workbench.addEventListener('click', (event) => {
        if (event.target === workbench)
            closeWorkbench();
    });
    document.addEventListener('keydown', (event) => {
        if (event.key === 'Escape' && workbench.classList.contains('nlk-open'))
            closeWorkbench();
    });
    // 聊天页右下角主入口。
    ...
    function updateFloat() {
        ...
        if (!state) {
            float.textContent = '万花筒 · 打开面板 · 未加载状态';
            runtimeStatus.textContent = '状态尚未加载';
            return;
        }
    ...
    try {
        ...
        eventSource.on('nlkaleido:contract_changed', () => {
            const contract = deps.adapter.adapter.contract;
            if (contract)
                store.contractText = json(contract);
            refresh();
        ...
        eventSource.on('nlkaleido:ejs_changed', refresh);
        eventSource.on('nlkaleido:runtime_error', (payload) => {
            store.runtimeMessage = String(payload?.message ?? payload ?? '未知运行时错误');
            updateFloat();
        });
    }
    catch (error) {
        panelBody.className = 'nlkaleido-workbench-body nlkaleido-mount-error';
        panelBody.textContent = `面板渲染失败：${error instanceof Error ? error.message : String(error)}`;
        openWorkbench();
        throw error;
    }
    // 设置页入口是增强项。容器异步出现时用 MutationObserver 补挂，不阻塞主界面。
    const attachSettingsEntry = () => {
        if (document.getElementById('nlkaleido_settings_entry'))
            return true;
        const host = document.getElementById('extensions_settings2')
            ?? document.getElementById('extensions_settings');
        if (!host)
            return false;
        host.appendChild(buildEntry(openWorkbench));
        updateFloat();
        return true;
    };
    if (!attachSettingsEntry()) {
    ...
    mountedPanel = { open: openWorkbench, close: closeWorkbench, refresh: updateFloat };
    // 首次安装主动展示一次，避免用户不知道右下角入口是什么。
    const seenKey = 'nlkaleido:workbench-seen';
    try {
    ...
    return {
        ...
        saveChat: async () => {
            ...
            if (typeof context.saveChat === 'function') {
                await context.saveChat(); // 立即落盘聊天文件；commit 只有在真实保存完成后才算成功
            }
        },
...
function start() {
    if (startupState === 'starting' || startupState === 'ready')
        return;
...
async function startAsync() {
    try {
        ...
        startupState = 'ready';
        publishRuntime();
        document.getElementById('nlkaleido_startup_error')?.remove();
    }
    catch (error) {
        startupState = 'failed';
        startupError = error instanceof Error ? `${error.name}: ${error.message}` : String(error);
        console.error('[NLKaleido] 启动失败：', error);
        publishRuntime();
...
async function startRuntime() {
    ...
    adapter.onRuntimeError = (message) => bridge.notifyRuntimeError(message);
    ...
    // M14 默认使用内建极简配置；完整配置/迁移实现仅在首次打开配置中心或执行配置动作时加载。
    await hydrateCurrentChat();
    // CHAT_CHANGED 后 chatMetadata 引用与 chat 级状态全部变化，必须重载。
    // 旧实现仅 APP_READY 水合一次，会把上一聊天的内存状态继续写入新聊天。
    const context = globals.getContext();
    const chatChanged = context.eventTypes.CHAT_CHANGED;
    if (typeof chatChanged === 'string') {
        context.eventSource.on(chatChanged, async () => {
            try {
                await hydrateCurrentChat();
            }
            catch (error) {
                adapter?.onRuntimeError?.(`聊天状态加载失败：${error instanceof Error ? error.message : String(error)}`);
            }
        });
    }
    // 面板挂载失败必须上浮到启动诊断，不能出现“核心在跑但用户以为没加载”。
    mountPanel({ bridge, adapter });
}
async function hydrateCurrentChat() {
    if (!adapter)
        return;
    const nextChatId = String(adapter.stGlobals.getContext().chatId ?? 'no-chat');
    if (previousChatId !== nextChatId) {
        void adapter.extensions.runGlobalHooks('onChatChanged', { chatId: nextChatId, prevChatId: previousChatId })
            .then((errors) => errors.forEach((error) => adapter?.onRuntimeError?.(`GlobalHook：${error}`)));
        previousChatId = nextChatId;
    }
    adapter.clearEjsInjection(true);
    const loaded = await adapter.loadStateAsync();
    ...
    if (contract.memory?.enabled) {
        ...
        await adapter.hydrateMemoryTables(contract);
    }
    else {
    ...
    if (contract.plot?.enabled)
        ...
        adapter.adapter.plotEvents = [];
        adapter.adapter.plotWinds = [];
        adapter.adapter.regionalIncident = { active: null, cooldown: 0 };
        adapter.adapter.plotRound = 0;
    }
    if (contract.dice?.enabled)
        adapter.loadDiceState();
    else {
        adapter.adapter.dicePresets = [];
        adapter.adapter.diceHistory = [];
    }
    if (bridge) {
        bridge.notifyContractChanged(contract.version);
        bridge.notifyStatusChanged(adapter.adapter.state);
        bridge.notifyMemoryChanged();
        bridge.notifyPlotChanged();
        bridge.notifyEjsChanged();
    }
}
function publishRuntime() {
    globalThis.NLKaleido = {
        version: VERSION,
        startupState,
        startupError,
        adapter,
        bridge,
        extensions: adapter ? {
            register: adapter.extensions.register.bind(adapter.extensions),
            use: adapter.extensions.use.bind(adapter.extensions),
            remove: adapter.extensions.remove.bind(adapter.extensions),
            list: adapter.extensions.list.bind(adapter.extensions),
            snapshot: adapter.extensions.snapshot.bind(adapter.extensions),
            resolve: adapter.extensions.resolve.bind(adapter.extensions),
        } : null,
        retry: () => window.location.reload(),
        getState: () => adapter?.adapter.state ?? null,
        getContract: () => adapter?.adapter.contract ?? null,
        variables: bridge ? {
            version: 1,
            get: (path) => bridge.getVariable(path),
            list: () => bridge.listVariables(),
            set: (path, value) => bridge.setVariable(path, value, 'frontend'),
            setMany: (values) => bridge.setVariables(values, 'frontend'),
            subscribe: (listener) => bridge.subscribeVariables(listener),
        } : null,
        diagnostics: adapter ? {
            /** 显式传 confirmCost:true 才会调用当前 provider；不会修改聊天状态。 */
            probeProviderCache: adapter.probeProviderCache.bind(adapter),
            downloadProviderCacheReport: async (options) => {
                const report = await adapter.probeProviderCache(options);
                const url = URL.createObjectURL(new Blob([JSON.stringify(report, null, 2)], { type: 'application/json' }));
                const link = document.createElement('a');
                link.href = url;
                link.download = `nlkaleido-provider-cache-${Date.now()}.json`;
                link.click();
        ...
        dispatch: (payload) => {
            if (!bridge)
                throw new Error('NLKaleido 尚未完成启动');
...
function showStartupError(message) {
    if (typeof document === 'undefined')
        return;
    let banner = document.getElementById('nlkaleido_startup_error');
    if (!banner) {
        banner = document.createElement('div');
        banner.id = 'nlkaleido_startup_error';
        Object.assign(banner.style, {
            position: 'fixed', right: '14px', bottom: '14px', zIndex: '50000', maxWidth: '520px',
            padding: '12px', border: '1px solid #f66', borderRadius: '8px', background: '#25191c', color: '#fff',
            boxShadow: '0 4px 24px rgba(0,0,0,.6)', whiteSpace: 'pre-wrap',
        });
        ...
        banner.append(document.createTextNode('NLKaleido 启动失败：'), retry);
        document.body.appendChild(banner);
    }
    const first = banner.firstChild;
    if (first)
        first.textContent = `NLKaleido 启动失败：${message} `;
}
...
// 若扩展模块早于 SillyTavern 全局可用，短期重试绑定，避免一次失败后永远不启动。
// ============================================================
function bindStartup(attempt = 0) {
    try {
        const context = getStContext();
        const readyEvent = context.eventTypes.APP_READY;
        if (typeof readyEvent !== 'string')
            throw new Error('eventTypes.APP_READY 缺失');
        context.eventSource.on(readyEvent, start);
    }
    catch (error) {
        if (attempt < 80) {
            window.setTimeout(() => bindStartup(attempt + 1), 250);
            return;
        }
        startupState = 'failed';
        startupError = error instanceof Error ? error.message : String(error);
        console.error('[NLKaleido] 无法绑定酒馆启动事件：', error);
        publishRuntime();
        showStartupError(startupError);
    }
}
...
export { estimateTokens as e, hash64 as h };
