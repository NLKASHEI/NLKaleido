...
class KaleidoStAdapter {
    ...
    async onGenerationEnded() {
        ...
        const result = await runSingleShotAgent(this, {
            ...
            turnId,
            l0: this.options.l0Template,
            recentStory: this.extractRecentStory(chat),
            userInput: this.extractUserInput(chat),
            jsonSchema: this.buildJsonSchema(this.adapter.contract),
        });
        if (result.requested) {
    ...
    /** 变量请求 json_schema（U4：字段名 json_schema；DeepSeek 来源走 G5 兜底） */
    buildJsonSchema(_contract) {
        return {
    ...
    async requestVariableUpdate(messages, jsonSchema) {
        ...
        return { analysis: parsed?.analysis, jsonPatch: parsed?.jsonPatch, usage: data?.usage };
    }
    ...
    onSettingsReady(generateData) {
        ...
        if (!generateData.json_schema) {
            generateData.json_schema = this.buildJsonSchema(this.adapter.contract ?? undefined);
        }
...
const useNlStore = defineStore('nlkaleido:panel', () => {
    ...
    return { compat, statData, pending, changelog, lastTurnId, authorMode, contractText, contractError };
});
function json(value) {
    ...
    setup() {
        ...
        return () => h('div', { class: 'nlk-root' }, [
            h('div', { class: 'nlk-tabs' }, [
                h('button', {
                    class: store.authorMode ? 'nlk-btn nlk-active' : 'nlk-btn',
                    onClick: () => { store.authorMode = !store.authorMode; },
                }, store.authorMode ? '作者模式' : '玩家模式'),
                store.authorMode ? h('span', { class: 'nlk-hint' }, '作者：契约/预览/调试') : null,
            ]),
            store.authorMode
...
function start() {
    ...
    // 加载契约 + 状态（chatMetadata；bootstrap 兜底，§4.4 恢复）
