...
- **配置（M14）**：“模型与运行”页可复用酒馆当前模型，或填写独立 OpenAI-compatible 主 AI；填写 URL 与 Key 后直接读取 `/models` 并下拉选择，只有接口不提供模型列表时才需手填。极简档固定 `1` 次传统变量更新并关闭记忆/世界推演；普通/进阶档可设置 `1～8` 次带请求间隔保护的串行 Agent，另有可选向量模型。
